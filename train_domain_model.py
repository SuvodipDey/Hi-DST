...
Finetunes bert-base-uncased model on domain prediction data.

Run Command:
python train_domain_model.py -in=<path of the input data> -path=<output dir> -src_file=<name of the python script>
"""
#--------------------------------------------
...
def prepare_data(df, max_len, batch_size, domain_dict):
    input_ids = []
    attention_masks = []
    ...
    dataset = TensorDataset(input_ids, attention_masks, domain_ids, labels)
...
def evaluate_metrics(dataloader, model):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            b_domain_ids = batch[2].to(device)
            labels = batch[3].to(device)
             
            outputs = model(b_input_ids, b_attn_mask, b_domain_ids)
...
train_dataloader = prepare_data(train_df, MAX_LEN, batch_size, domain_dict)
print("Loading Test data")
test_dataloader = prepare_data(test_df, MAX_LEN, batch_size, domain_dict)
print("Loading Validation data")
valid_dataloader = prepare_data(valid_df, MAX_LEN, batch_size, domain_dict)
...
model = Model(domain_matrix, num_labels)
model.to(device)
...
best_train_loss = 0
best_model = 0
model_copy = type(model)(domain_matrix, num_labels)
...
PATH = os.path.join(work_dir , 'domain_model.pt')
